...
async def purchase_order_workflow(ctx, order: Order):
    ...
    # In async API, when_any returns a proxy that compares equal to the original awaitable
    winner = await ctx.when_any([approval, timeout])
    if winner == timeout:
        return 'Cancelled'

    await ctx.activity(place_order, input=order)
    # The proxy exposes get_result() to read the payload
    approval_details = winner.get_result()
