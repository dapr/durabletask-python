...
async def enhanced_workflow_example(ctx: AsyncWorkflowContext, config: dict) -> dict:
    ...
    async with ctx:
        try:
            ...
            print(
                f"[{ctx.instance_id}] Running activity with {timeout_seconds}s timeout"
            )
            ...
            completed_index, completed_result = await ctx.when_any_with_result(
                activities
            )
            print(
                f"[{ctx.instance_id}] Task {completed_index} completed first: {completed_result}"
            )
            ...
            print(
                f"[{ctx.instance_id}] Successes: {len(successes)}, Failures: {len(failures)}"
            )
...
def main():
    ...
    with TaskHubGrpcWorker() as worker:
        ...
        instance_id = client.schedule_new_orchestration(
            "enhanced_workflow", input=test_config
        )
        ...
        def send_event():
            ...
            try:
                client.raise_event(
                    instance_id, "user_input", "Hello from external event!"
                )
