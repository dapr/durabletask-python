...
class _NonDeterminismDetector:
    ...
    def _noop_trace(self, frame: FrameType, event: str, arg: Any) -> Optional[Callable[[FrameType, str, Any], Any]]:  # lightweight tracer
        return None
        
    def __enter__(self) -> "_NonDeterminismDetector":
        enable_full_detection = (
            self.mode == "strict"
            or (self.mode == "best_effort" and getattr(self.async_ctx, "_debug_mode", False))
        )
        if self.mode in ("best_effort", "strict"):
            ...
            self._active_trace_func = self._trace_calls if enable_full_detection else self._noop_trace
    ...
    def _trace_calls(self, frame: FrameType, event: str, arg: Any) -> Optional[Callable[[FrameType, str, Any], Any]]:
    ...
    def _check_frame_for_non_determinism(self, frame: FrameType) -> None:
        ...
        if module_name:
            ...
            if funcs and func_name in funcs:
                ...
                if module_name == "random" and func_name in {"random", "randint", "choice", "shuffle"}:
                    try:
        ...
        if self.mode != "best_effort":
            ...
            for pattern_module, pattern_func in non_deterministic_patterns:
                ...
                try:
                    if isinstance(globals_map, dict) and full_name in globals_map and func_name == pattern_func:
...
class _Sandbox(ContextDecorator):
    ...
    def _apply_patches(self) -> None:
        ...
        def patched_sleep(delay: Union[float, int]) -> Any:
            ...
            class _PatchedSleepAwaitable:
                def __await__(self) -> Any:
                    result = yield base_ctx.create_timer(
        ...
        def patched_randrange(start: int, stop: Optional[int] = None, step: int = 1, _int: Callable[[float], int] = int) -> int:
