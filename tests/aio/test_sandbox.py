...
class TestUnsafeWallClockNow:
    """Tests for AsyncWorkflowContext.unsafe_wall_clock_now."""

    def test_unsafe_wall_clock_now_best_effort(self):
        from datetime import timezone

        from durabletask.aio import AsyncWorkflowContext

        base_ctx = Mock()
        base_ctx.instance_id = "unsafe-now"
        base_ctx.current_utc_datetime = datetime.datetime(2023, 1, 1, 12, 0, 0)
        base_ctx.is_replaying = False
        async_ctx = AsyncWorkflowContext(base_ctx)

        with sandbox_scope(async_ctx, "best_effort"):
            ts = async_ctx.unsafe_wall_clock_now()
            assert isinstance(ts, datetime.datetime)
            # Should be close to real wall clock UTC
            real = datetime.datetime.now(timezone.utc)
            assert abs((real - ts).total_seconds()) < 5

    def test_unsafe_wall_clock_now_strict_raises(self):
        from durabletask.aio import AsyncWorkflowContext

        base_ctx = Mock()
        base_ctx.instance_id = "unsafe-now-strict"
        base_ctx.current_utc_datetime = datetime.datetime(2023, 1, 1, 12, 0, 0)
        base_ctx.is_replaying = False
        async_ctx = AsyncWorkflowContext(base_ctx)

        with sandbox_scope(async_ctx, "strict"):
            with pytest.raises(RuntimeError, match="disabled in strict sandbox"):
                _ = async_ctx.unsafe_wall_clock_now()

    def test_unsafe_wall_clock_now_replay_raises(self):
        from durabletask.aio import AsyncWorkflowContext

        base_ctx = Mock()
        base_ctx.instance_id = "unsafe-now-replay"
        base_ctx.current_utc_datetime = datetime.datetime(2023, 1, 1, 12, 0, 0)
        base_ctx.is_replaying = True
        async_ctx = AsyncWorkflowContext(base_ctx)

        with sandbox_scope(async_ctx, "best_effort"):
            with pytest.raises(RuntimeError, match="cannot be used during replay"):
                _ = async_ctx.unsafe_wall_clock_now()
