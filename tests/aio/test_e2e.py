...
def _log_orchestration_progress(hub_client: TaskHubGrpcClient, instance_id: str, max_seconds: int = 60) -> None:
...
class TestAsyncWorkflowE2E:
    ...
    def _register_test_functions(cls):
        ...
        async def parent_async_workflow(ctx: AsyncWorkflowContext, input_data: str) -> dict:
            ...
            child_result = await ctx.call_sub_orchestrator(
                child_async_workflow,
                input=input_data,
                instance_id=f"{ctx.instance_id}_child"
            )
    ...
    def test_simple_async_workflow_e2e(self):
        ...
        try:
            ...
            print(f"[async e2e] server-side wait timed out after {elapsed:.1f}s; polling for remaining {remaining}s")
            if remaining > 0:
    ...
    async def test_multi_step_async_workflow_e2e(self):
        ...
        self.client.schedule_new_orchestration(
            type(self).multi_step_async_workflow,
            input=3,
            instance_id=instance_id
        )
        
        # Wait for completion
        result = self.client.wait_for_orchestration_completion(
            instance_id,
            timeout=30
        )
    ...
    async def test_parallel_async_workflow_e2e(self):
        ...
        # Start workflow
        self.client.schedule_new_orchestration(
            type(self).parallel_async_workflow,
            input=3,
            instance_id=instance_id
        )
    ...
    async def test_timer_async_workflow_e2e(self):
        ...
        # Start workflow
        self.client.schedule_new_orchestration(
            type(self).timer_async_workflow,
            input=delay_seconds,
            instance_id=instance_id
        )
    ...
    async def test_sub_orchestrator_async_workflow_e2e(self):
        ...
        self.client.schedule_new_orchestration(
            type(self).parent_async_workflow,
            input="test_data",
            instance_id=instance_id
        )
    ...
    async def test_when_any_activities_e2e(self):
        ...
        self.client.schedule_new_orchestration(type(self).when_any_activities, input=None, instance_id=instance_id)
        ...
        if result.failure_details:
            print("when_any_activities failure:", result.failure_details.error_type, result.failure_details.message)
    ...
    async def test_when_any_with_timer_e2e(self):
        ...
        self.client.schedule_new_orchestration(type(self).when_any_with_timer, input=None, instance_id=instance_id)
        try:
    ...
    async def test_when_any_event_or_timeout_e2e(self):
        ...
        event_name = "evt"
        self.client.schedule_new_orchestration(type(self).when_any_event_or_timeout, input=event_name, instance_id=instance_id)
        try:
            _ = self.client.wait_for_orchestration_start(instance_id, timeout=30)
        except Exception:
            pass
        ...
        assert result is not None
        if result.failure_details:
            print("when_any_event_or_timeout failure:", result.failure_details.error_type, result.failure_details.message)
    ...
    async def test_async_workflow_error_handling_e2e(self):
        ...
        # Start pre-registered workflow
        self.client.schedule_new_orchestration(
            type(self).error_handling_workflow,
            input="test_error_input",
            instance_id=instance_id
        )
    ...
    async def test_async_workflow_with_external_event_e2e(self):
        ...
        # Start pre-registered workflow
        self.client.schedule_new_orchestration(
            type(self).external_event_workflow,
            input="test_event",
            instance_id=instance_id
        )
