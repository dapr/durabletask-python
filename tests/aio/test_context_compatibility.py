...
class TestAsyncWorkflowContextCompatibility:
    ...
    def setup_method(self):
        ...
        self.mock_base_ctx.trace_parent = "00-trace-parent-00"
        self.mock_base_ctx.trace_state = "trace-state"
        self.mock_base_ctx.orchestration_span_id = "span-123"
    ...
    def test_all_orchestration_context_properties_exist(self):
        ...
        for prop_name in orchestration_properties:
            ...
            assert prop_value is not None or prop_name in ['parent_instance_id', 'trace_parent', 'trace_state', 'orchestration_span_id'], \
    ...
    def test_property_compatibility_trace_properties(self):
        """Test trace-related properties compatibility."""
        assert self.async_ctx.trace_parent == "00-trace-parent-00"
        assert self.async_ctx.trace_state == "trace-state"
        assert self.async_ctx.orchestration_span_id == "span-123"
    ...
    def test_method_compatibility_call_sub_orchestrator(self):
        ...
        sub_orch_awaitable = self.async_ctx.call_sub_orchestrator(
            test_orchestrator, 
            input="test"
        )
    ...
    def test_method_signature_compatibility(self):
        ...
        base_methods = {}
        for name, method in inspect.getmembers(task.OrchestrationContext, predicate=inspect.isfunction):
    ...
    def test_return_type_compatibility(self):
        ...
        assert hasattr(activity_result, '__await__'), \
            "call_activity should return an awaitable"
        
        # Test that timer calls return awaitables
        timer_result = self.async_ctx.create_timer(timedelta(seconds=1))
        assert hasattr(timer_result, '__await__'), \
            "create_timer should return an awaitable"
