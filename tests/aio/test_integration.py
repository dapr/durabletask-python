...
class FakeCtx:
    ...
    def call_sub_orchestrator(self, orchestrator, *, input=None, instance_id=None, retry_policy=None, metadata=None):
...
class TestAsyncWorkflowIntegration:
    ...
    def test_sub_orchestrator_workflow_integration(self):
        ...
        assert result == {
            "sub_result": "sub_orchestrator_result",
            "final": "final_result"
        }
    ...
    def test_external_event_workflow_integration(self):
        ...
        async def event_workflow(ctx: AsyncWorkflowContext, event_name: str) -> dict:
            ...
            # Process event data
            final_result = await ctx.call_activity("process_event", input={
                "start": start_result,
                "event": event_data
            })
    ...
    def test_deterministic_operations_integration(self):
        ...
        try:
            ...
            # Verify deterministic behavior - run again with same context
            gen2 = runner.to_generator(async_ctx, 3)
            try:
                next(gen2)
